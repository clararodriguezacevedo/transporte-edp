#PRUEBA CON UN ATRIBUTO CONEXIONES EN NODO QUE ES UN DICC, Y HACIENDO UN INTENTO DE GRAFO
...
class Nodo:
    def __init__(self,ciudad):
        ...
        #tal vez sea mejor poner distintos atributos? tipo self.ferroviaria, self.fluvial, etc
        
    def __str__(self):
        return f"Ciudad: {self.ciudad}"
    
    def enlazar_conexion(self,conexion): #agrega la conexion de este nodo al diccionario
        if not isinstance(conexion,Conexion):
            raise ValueError("No se ingreso una conexion")
        self.conexiones[conexion.tipo].append(conexion)
    
    def mostrar_info(self): #muestra la ciudad del nodo, y todas sus conexiones
        print(f"Ciudad: {self.ciudad}")
        for key,value in self.conexiones.items():
            print(f"Modo {key}:")
            if value!=[]:
                for elemento in value:
                    print(elemento)
            else:
                print("No hay conexiones en este metodo")
